...
def filter_obstickles_close(grid : List[List[int]], my_head: Dict[str, int], possible_moves: List[str],board_height,board_width):
  """
  ...
  for move in possible_moves:
    ...
    if my_x + x == board_width or my_x + x < 0: 
      cur_moves.remove(move)
      continue
    if grid[board_height - 1 - my_y - y][my_x + x] != 0:# TODO change this is information in the grid is no longer a int
      cur_moves.remove(move)
  if len(cur_moves) == 0:
    return ["left"]
  return cur_moves
...
def choose_move(data: dict) -> str:
    ...
    possible_moves = filter_obstickles_close(our_board, my_head, possible_moves, board_height, board_width)
    # print(my_head)
    # print(board_height)
